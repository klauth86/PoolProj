...
const FName UMyArmComponent::SocketName(TEXT("SpringEndpoint"));

UMyArmComponent::UMyArmComponent(const FObjectInitializer& ObjectInitializer)
...
FRotator UMyArmComponent::GetTargetRotation() const {
...
void UMyArmComponent::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime) {
...
FVector UMyArmComponent::BlendLocations(const FVector& DesiredArmLocation, const FVector& TraceHitLocation, bool bHitSomething, float DeltaTime) {
	return bHitSomething ? TraceHitLocation : DesiredArmLocation;
}

void UMyArmComponent::ApplyWorldOffset(const FVector & InOffset, bool bWorldShift) {
	Super::ApplyWorldOffset(InOffset, bWorldShift);
	PreviousDesiredLoc += InOffset;
	PreviousArmOrigin += InOffset;
}

void UMyArmComponent::OnRegister() {
	Super::OnRegister();
...
void UMyArmComponent::PostLoad() {
	Super::PostLoad();
}

void UMyArmComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction) {
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);
	UpdateDesiredArmLocation(bDoCollisionTest, bEnableCameraLag, bEnableCameraRotationLag, DeltaTime);
}

FTransform UMyArmComponent::GetSocketTransform(FName InSocketName, ERelativeTransformSpace TransformSpace) const {
...
bool UMyArmComponent::HasAnySockets() const {
	return true;
}

void UMyArmComponent::QuerySupportedSockets(TArray<FComponentSocketDescription>& OutSockets) const {
	new (OutSockets) FComponentSocketDescription(SocketName, EComponentSocketType::Socket);
}

FVector UMyArmComponent::GetUnfixedCameraPosition() const {
	return UnfixedCameraPosition;
}

bool UMyArmComponent::IsCollisionFixApplied() const {
