...
FString FCharacterMovementComponentPostPhysicsTickFunction::DiagnosticMessage() {
	return Target->GetFullName() + TEXT("[UMyPawnMovementComponent::PreClothTick]");
}
...
void UMyPawnMovementComponent::PostLoad() {
	Super::PostLoad();
	...
	if (LinkerUE4Ver < VER_UE4_CHARACTER_MOVEMENT_WALKABLE_FLOOR_REFACTOR) {
		...
		UMyPawnMovementComponent::SetWalkableFloorZ(WalkableFloorZ);
	}
...
#if WITH_EDITOR
void UMyPawnMovementComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent) {
...
void UMyPawnMovementComponent::OnRegister() {
...
void UMyPawnMovementComponent::BeginDestroy() {
...
void UMyPawnMovementComponent::Deactivate() {
...
void UMyPawnMovementComponent::SetUpdatedComponent(USceneComponent* NewUpdatedComponent) {
	...
	if (IsValid(OldPrimitive) && OldPrimitive->OnComponentBeginOverlap.IsBound()) {
		OldPrimitive->OnComponentBeginOverlap.RemoveDynamic(this, &UMyPawnMovementComponent::CapsuleTouched);
	}
	...
	if (bValidUpdatedPrimitive && bEnablePhysicsInteraction) {
		UpdatedPrimitive->OnComponentBeginOverlap.AddUniqueDynamic(this, &UMyPawnMovementComponent::CapsuleTouched);
	}
...
bool UMyPawnMovementComponent::HasValidData() const {
	bool bIsValid = UpdatedComponent && IsValid(CharacterOwner);
#if ENABLE_NAN_DIAGNOSTIC
	if (bIsValid) {
		// NaN-checking updates
		if (Velocity.ContainsNaN()) {
			logOrEnsureNanError(TEXT("UMyPawnMovementComponent::HasValidData() detected NaN/INF for (%s) in Velocity:\n%s"), *GetPathNameSafe(this), *Velocity.ToString());
		...
		if (!UpdatedComponent->GetComponentTransform().IsValid()) {
			logOrEnsureNanError(TEXT("UMyPawnMovementComponent::HasValidData() detected NaN/INF for (%s) in UpdatedComponent->ComponentTransform:\n%s"), *GetPathNameSafe(this), *UpdatedComponent->GetComponentTransform().ToHumanReadableString());
		}
		if (UpdatedComponent->GetComponentRotation().ContainsNaN()) {
			logOrEnsureNanError(TEXT("UMyPawnMovementComponent::HasValidData() detected NaN/INF for (%s) in UpdatedComponent->GetComponentRotation():\n%s"), *GetPathNameSafe(this), *UpdatedComponent->GetComponentRotation().ToString());
		}
	}
#endif
	return bIsValid;
...
FCollisionShape UMyPawnMovementComponent::GetPawnCapsuleCollisionShape(const EShrinkCapsuleExtent ShrinkMode, const float CustomShrinkAmount) const {
	FVector Extent = GetPawnCapsuleExtent(ShrinkMode, CustomShrinkAmount);
	return FCollisionShape::MakeCapsule(Extent);
}

FVector UMyPawnMovementComponent::GetPawnCapsuleExtent(const EShrinkCapsuleExtent ShrinkMode, const float CustomShrinkAmount) const {
	...
	default:
		UE_LOG(LogCharacterMovement, Warning, TEXT("Unknown EShrinkCapsuleExtent in UMyPawnMovementComponent::GetCapsuleExtent"));
		break;
	}
...
bool UMyPawnMovementComponent::DoJump(bool bReplayingMoves) {
...
FVector UMyPawnMovementComponent::GetImpartedMovementBaseVelocity() const {
...
void UMyPawnMovementComponent::Launch(FVector const& LaunchVel) {
	if ((MovementMode != MOVE_None) && IsActive() && HasValidData()) {
		PendingLaunchVelocity = LaunchVel;
	}
}

bool UMyPawnMovementComponent::HandlePendingLaunch() {
...
void UMyPawnMovementComponent::JumpOff(AActor* MovementBaseActor) {
...
FVector UMyPawnMovementComponent::GetBestDirectionOffActor(AActor* BaseActor) const {
...
float UMyPawnMovementComponent::GetNetworkSafeRandomAngleDegrees() const {
...
void UMyPawnMovementComponent::SetDefaultMovementMode() {
...
void UMyPawnMovementComponent::SetGroundMovementMode(EMovementMode NewGroundMovementMode) {
...
void UMyPawnMovementComponent::SetMovementMode(EMovementMode NewMovementMode, uint8 NewCustomMode) {
...
void UMyPawnMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode) {
	if (!HasValidData()) {
		return;
	}
...
uint8 UMyPawnMovementComponent::PackNetworkMovementMode() const {
...
void UMyPawnMovementComponent::UnpackNetworkMovementMode(const uint8 ReceivedMode, TEnumAsByte<EMovementMode>& OutMode, uint8& OutCustomMode, TEnumAsByte<EMovementMode>& OutGroundMode) const {
...
void UMyPawnMovementComponent::ApplyNetworkMovementMode(const uint8 ReceivedMode) {
...
void UMyPawnMovementComponent::PerformAirControlForPathFollowing(FVector Direction, float ZDiff) {
...
void UMyPawnMovementComponent::Serialize(FArchive& Archive) {
...
void UMyPawnMovementComponent::PostPhysicsTickComponent(float DeltaTime, FCharacterMovementComponentPostPhysicsTickFunction& ThisTickFunction) {
...
void UMyPawnMovementComponent::AdjustProxyCapsuleSize() {
...
void UMyPawnMovementComponent::SimulatedTick(float DeltaSeconds) {
	...
	if (CharacterOwner->IsPlayingNetworkedRootMotionMontage()) {
		...
		UE_LOG(LogRootMotion, Verbose, TEXT("UMyPawnMovementComponent::SimulatedTick"));
		...
		if (RootMotionParams.bHasRootMotion) {
			...
			if (false) {
				...
				UE_LOG(LogRootMotion, Log, TEXT("UMyPawnMovementComponent::SimulatedTick DeltaMovement Translation: %s, Rotation: %s, MovementBase: %s"),
...
FTransform UMyPawnMovementComponent::ConvertLocalRootMotionToWorld(const FTransform& LocalRootMotionTransform) {
...
void UMyPawnMovementComponent::SimulateRootMotion(float DeltaSeconds, const FTransform& LocalRootMotionTransform) {
...
FVector UMyPawnMovementComponent::CalcRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaSeconds, const FVector& CurrentVelocity) const {
	return CalcAnimRootMotionVelocity(RootMotionDeltaMove, DeltaSeconds, CurrentVelocity);
}


FVector UMyPawnMovementComponent::CalcAnimRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaSeconds, const FVector& CurrentVelocity) const {
...
FVector UMyPawnMovementComponent::ConstrainAnimRootMotionVelocity(const FVector& RootMotionVelocity, const FVector& CurrentVelocity) const {
...
void UMyPawnMovementComponent::SimulateMovement(float DeltaSeconds) {
...
UPrimitiveComponent* UMyPawnMovementComponent::GetMovementBase() const {
	return CharacterOwner ? CharacterOwner->GetMovementBase() : NULL;
}

void UMyPawnMovementComponent::SetBase(UPrimitiveComponent* NewBase, FName BoneName, bool bNotifyActor) {
...
void UMyPawnMovementComponent::SetBaseFromFloor(const FFindFloorResult& FloorResult) {
...
void UMyPawnMovementComponent::MaybeUpdateBasedMovement(float DeltaSeconds) {
	bDeferUpdateBasedMovement = false;
...
void UMyPawnMovementComponent::MaybeSaveBaseLocation() {
	if (!bDeferUpdateBasedMovement) {
		SaveBaseLocation();
	}
}
...
void UMyPawnMovementComponent::UpdateBasedMovement(float DeltaSeconds) {
	if (!HasValidData()) {
		return;
	}
...
void UMyPawnMovementComponent::OnUnableToFollowBaseMove(const FVector& DeltaPosition, const FVector& OldLocation, const FHitResult& MoveOnBaseHit) {
	// no default implementation, left for subclasses to override.
}


void UMyPawnMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation) {
...
void UMyPawnMovementComponent::DisableMovement() {
	if (CharacterOwner) {
...
void UMyPawnMovementComponent::PerformMovement(float DeltaSeconds) {
	...
	// Scoped updates can improve performance of multiple MoveComponent calls.
	{
		...
		// NaN tracking
		checkCode(ensureMsgf(!Velocity.ContainsNaN(), TEXT("UMyPawnMovementComponent::PerformMovement: Velocity contains NaN (%s)\n%s"), *GetPathNameSafe(this), *Velocity.ToString()));
...
bool UMyPawnMovementComponent::ShouldCancelAdaptiveReplication() const {
...
void UMyPawnMovementComponent::CallMovementUpdateDelegate(float DeltaTime, const FVector& OldLocation, const FVector& OldVelocity) {
...
void UMyPawnMovementComponent::OnMovementUpdated(float DeltaTime, const FVector& OldLocation, const FVector& OldVelocity) {
	// empty base implementation, intended for derived classes to override.
}


void UMyPawnMovementComponent::SaveBaseLocation() {
	if (!HasValidData()) {
		return;
	}
...
bool UMyPawnMovementComponent::CanCrouchInCurrentState() const {
	if (!CanEverCrouch()) {
		return false;
	}
...
void UMyPawnMovementComponent::Crouch(bool bClientSimulation) {
	if (!HasValidData()) {
		return;
	}
...
void UMyPawnMovementComponent::UnCrouch(bool bClientSimulation) {
	if (!HasValidData()) {
		return;
	}
...
void UMyPawnMovementComponent::UpdateCharacterStateBeforeMovement() {
...
void UMyPawnMovementComponent::UpdateCharacterStateAfterMovement() {
...
void UMyPawnMovementComponent::StartNewPhysics(float deltaTime, int32 Iterations) {
	if ((deltaTime < MIN_TICK_TIME) || (Iterations >= MaxSimulationIterations) || !HasValidData()) {
		return;
	}

	if (UpdatedComponent->IsSimulatingPhysics()) {
		UE_LOG(LogCharacterMovement, Log, TEXT("UMyPawnMovementComponent::StartNewPhysics: UpdateComponent (%s) is simulating physics - aborting."), *UpdatedComponent->GetPathName());
		return;
	}
...
float UMyPawnMovementComponent::GetGravityZ() const {
	return Super::GetGravityZ() * GravityScale;
}

float UMyPawnMovementComponent::GetMaxSpeed() const {
	switch (MovementMode) {
	case MOVE_Walking:
	case MOVE_NavWalking:
...
float UMyPawnMovementComponent::GetMinAnalogSpeed() const {
	switch (MovementMode) {
	case MOVE_Walking:
	case MOVE_NavWalking:
	case MOVE_Falling:
...
FVector UMyPawnMovementComponent::GetPenetrationAdjustment(const FHitResult& Hit) const {
...
bool UMyPawnMovementComponent::ResolvePenetrationImpl(const FVector& Adjustment, const FHitResult& Hit, const FQuat& NewRotation) {
...
float UMyPawnMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& InNormal, FHitResult& Hit, bool bHandleImpact) {
	if (!Hit.bBlockingHit) {
		return 0.f;
	}
...
void UMyPawnMovementComponent::TwoWallAdjust(FVector& Delta, const FHitResult& Hit, const FVector& OldHitNormal) const {
...
FVector UMyPawnMovementComponent::ComputeSlideVector(const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const {
...
FVector UMyPawnMovementComponent::HandleSlopeBoosting(const FVector& SlideResult, const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const {
...
FVector UMyPawnMovementComponent::NewFallVelocity(const FVector& InitialVelocity, const FVector& Gravity, float DeltaTime) const {
...
float UMyPawnMovementComponent::ImmersionDepth() const {
	float depth = 0.f;
...
bool UMyPawnMovementComponent::IsFlying() const {
	return (MovementMode == MOVE_Flying) && UpdatedComponent;
}

bool UMyPawnMovementComponent::IsMovingOnGround() const {
	return ((MovementMode == MOVE_Walking) || (MovementMode == MOVE_NavWalking)) && UpdatedComponent;
}

bool UMyPawnMovementComponent::IsFalling() const {
	return (MovementMode == MOVE_Falling) && UpdatedComponent;
}

bool UMyPawnMovementComponent::IsSwimming() const {
	return (MovementMode == MOVE_Swimming) && UpdatedComponent;
}

bool UMyPawnMovementComponent::IsCrouching() const {
	return CharacterOwner && CharacterOwner->bIsCrouched;
}

void UMyPawnMovementComponent::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration) {
...
bool UMyPawnMovementComponent::ApplyRequestedMove(float DeltaTime, float MaxAccel, float MaxSpeed, float Friction, float BrakingDeceleration, FVector& OutAcceleration, float& OutRequestedSpeed) {
...
void UMyPawnMovementComponent::RequestDirectMove(const FVector& MoveVelocity, bool bForceMaxSpeed) {
...
void UMyPawnMovementComponent::RequestPathMove(const FVector& MoveInput) {
...
bool UMyPawnMovementComponent::CanStartPathFollowing() const {
...
bool UMyPawnMovementComponent::CanStopPathFollowing() const {
	return !IsFalling();
}

float UMyPawnMovementComponent::GetPathFollowingBrakingDistance(float MaxSpeed) const {
...
void UMyPawnMovementComponent::CalcAvoidanceVelocity(float DeltaTime) {
...
void UMyPawnMovementComponent::PostProcessAvoidanceVelocity(FVector& NewVelocity) {
	// empty in base class
}

void UMyPawnMovementComponent::UpdateDefaultAvoidance() {
	if (!bUseRVOAvoidance) {
		return;
	}
...
void UMyPawnMovementComponent::SetRVOAvoidanceUID(int32 UID) {
	AvoidanceUID = UID;
}

int32 UMyPawnMovementComponent::GetRVOAvoidanceUID() {
	return AvoidanceUID;
}

void UMyPawnMovementComponent::SetRVOAvoidanceWeight(float Weight) {
	AvoidanceWeight = Weight;
}

float UMyPawnMovementComponent::GetRVOAvoidanceWeight() {
	return AvoidanceWeight;
}

FVector UMyPawnMovementComponent::GetRVOAvoidanceOrigin() {
	return GetActorFeetLocation();
}

float UMyPawnMovementComponent::GetRVOAvoidanceRadius() {
...
float UMyPawnMovementComponent::GetRVOAvoidanceConsiderationRadius() {
	return AvoidanceConsiderationRadius;
}

float UMyPawnMovementComponent::GetRVOAvoidanceHeight() {
	UCapsuleComponent* CapsuleComp = GetCharacterOwner()->GetCapsuleComponent();
	return CapsuleComp ? CapsuleComp->GetScaledCapsuleHalfHeight() : 0.0f;
}

FVector UMyPawnMovementComponent::GetVelocityForRVOConsideration() {
	return Velocity;
}

int32 UMyPawnMovementComponent::GetAvoidanceGroupMask() {
	return AvoidanceGroup.Packed;
}

int32 UMyPawnMovementComponent::GetGroupsToAvoidMask() {
	return GroupsToAvoid.Packed;
}

int32 UMyPawnMovementComponent::GetGroupsToIgnoreMask() {
	return GroupsToIgnore.Packed;
}

void UMyPawnMovementComponent::SetAvoidanceVelocityLock(class UAvoidanceManager* Avoidance, float Duration) {
	Avoidance->OverrideToMaxWeight(AvoidanceUID, Duration);
	AvoidanceLockVelocity = Velocity;
	AvoidanceLockTimer = Duration;
}

void UMyPawnMovementComponent::NotifyBumpedPawn(APawn* BumpedPawn) {
...
float UMyPawnMovementComponent::GetMaxJumpHeight() const {
...
float UMyPawnMovementComponent::GetMaxJumpHeightWithJumpTime() const {
...
// TODO: deprecated, remove.
float UMyPawnMovementComponent::GetModifiedMaxAcceleration() const {
...
// TODO: deprecated, remove.
float UMyPawnMovementComponent::K2_GetModifiedMaxAcceleration() const {
	// Allow calling old deprecated function to maintain old behavior until it is removed.
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
float UMyPawnMovementComponent::GetMaxAcceleration() const {
	return MaxAcceleration;
}

float UMyPawnMovementComponent::GetMaxBrakingDeceleration() const {
	switch (MovementMode) {
	case MOVE_Walking:
	case MOVE_NavWalking:
...
FVector UMyPawnMovementComponent::GetCurrentAcceleration() const {
	return Acceleration;
}

void UMyPawnMovementComponent::ApplyVelocityBraking(float DeltaTime, float Friction, float BrakingDeceleration) {
...
void UMyPawnMovementComponent::PhysFlying(float deltaTime, int32 Iterations) {
...
void UMyPawnMovementComponent::RestorePreAdditiveRootMotionVelocity() {
...
void UMyPawnMovementComponent::ApplyRootMotionToVelocity(float deltaTime) {
	SCOPE_CYCLE_COUNTER(STAT_CharacterMovementRootMotionSourceApply);
...
void UMyPawnMovementComponent::HandleSwimmingWallHit(const FHitResult& Hit, float DeltaTime) {}

void UMyPawnMovementComponent::PhysSwimming(float deltaTime, int32 Iterations) {
	if (deltaTime < MIN_TICK_TIME) {
		return;
	}
...
void UMyPawnMovementComponent::StartSwimming(FVector OldLocation, FVector OldVelocity, float timeTick, float remainingTime, int32 Iterations) {
...
float UMyPawnMovementComponent::Swim(FVector Delta, FHitResult& Hit) {
...
FVector UMyPawnMovementComponent::FindWaterLine(FVector InWater, FVector OutofWater) {
...
void UMyPawnMovementComponent::NotifyJumpApex() {
	if (CharacterOwner) {
...
FVector UMyPawnMovementComponent::GetFallingLateralAcceleration(float DeltaTime) {
...
FVector UMyPawnMovementComponent::GetAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration) {
	// Boost
...
float UMyPawnMovementComponent::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration) {
...
void UMyPawnMovementComponent::PhysFalling(float deltaTime, int32 Iterations) {
...
FVector UMyPawnMovementComponent::LimitAirControl(float DeltaTime, const FVector& FallAcceleration, const FHitResult& HitResult, bool bCheckForValidLandingSpot) {
...
bool UMyPawnMovementComponent::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir) const {
...
FVector UMyPawnMovementComponent::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir) const {
...
bool UMyPawnMovementComponent::CanWalkOffLedges() const {
...
bool UMyPawnMovementComponent::CheckFall(const FFindFloorResult& OldFloor, const FHitResult& Hit, const FVector& Delta, const FVector& OldLocation, float remainingTime, float timeTick, int32 Iterations, bool bMustJump) {
	if (!HasValidData()) {
		return false;
	}
...
void UMyPawnMovementComponent::StartFalling(int32 Iterations, float remainingTime, float timeTick, const FVector& Delta, const FVector& subLoc) {
	// start falling 
	const float DesiredDist = Delta.Size();
...
void UMyPawnMovementComponent::RevertMove(const FVector& OldLocation, UPrimitiveComponent* OldBase, const FVector& PreviousBaseLocation, const FFindFloorResult& OldFloor, bool bFailMove) {
...
FVector UMyPawnMovementComponent::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace) const {
...
void UMyPawnMovementComponent::OnCharacterStuckInGeometry(const FHitResult* Hit) {
...
void UMyPawnMovementComponent::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FStepDownResult* OutStepDownResult) {
	if (!CurrentFloor.IsWalkableFloor()) {
		return;
	}
...
void UMyPawnMovementComponent::MaintainHorizontalGroundVelocity() {
	if (Velocity.Z != 0.f) {
		if (bMaintainHorizontalGroundVelocity) {
...
void UMyPawnMovementComponent::PhysWalking(float deltaTime, int32 Iterations) {
...
void UMyPawnMovementComponent::PhysNavWalking(float deltaTime, int32 Iterations) {
...
bool UMyPawnMovementComponent::FindNavFloor(const FVector& TestLocation, FNavLocation& NavFloorLocation) const {
...
FVector UMyPawnMovementComponent::ProjectLocationFromNavMesh(float DeltaSeconds, const FVector& CurrentFeetLocation, const FVector& TargetNavLocation, float UpOffset, float DownOffset) {
	SCOPE_CYCLE_COUNTER(STAT_CharNavProjectLocation);
...
void UMyPawnMovementComponent::FindBestNavMeshLocation(const FVector& TraceStart, const FVector& TraceEnd, const FVector& CurrentFeetLocation, const FVector& TargetNavLocation, FHitResult& OutHitResult) const {
...
const ANavigationData* UMyPawnMovementComponent::GetNavData() const {
...
void UMyPawnMovementComponent::PhysCustom(float deltaTime, int32 Iterations) {
	if (CharacterOwner) {
...
bool UMyPawnMovementComponent::ShouldCatchAir(const FFindFloorResult& OldFloor, const FFindFloorResult& NewFloor) {
	return false;
}

void UMyPawnMovementComponent::AdjustFloorHeight() {
...
void UMyPawnMovementComponent::StopActiveMovement() {
...
void UMyPawnMovementComponent::ProcessLanded(const FHitResult& Hit, float remainingTime, int32 Iterations) {
...
void UMyPawnMovementComponent::SetPostLandedPhysics(const FHitResult& Hit) {
	if (CharacterOwner) {
		if (CanEverSwim() && IsInWater()) {
...
void UMyPawnMovementComponent::SetNavWalkingPhysics(bool bEnable) {
	if (UpdatedPrimitive) {
		if (bEnable) {
...
bool UMyPawnMovementComponent::TryToLeaveNavWalking() {
	SetNavWalkingPhysics(false);
...
void UMyPawnMovementComponent::OnTeleported() {
	if (!HasValidData()) {
		return;
	}
...
FRotator UMyPawnMovementComponent::GetDeltaRotation(float DeltaTime) const {
...
FRotator UMyPawnMovementComponent::ComputeOrientToMovementRotation(const FRotator& CurrentRotation, float DeltaTime, FRotator& DeltaRotation) const {
...
bool UMyPawnMovementComponent::ShouldRemainVertical() const {
	// Always remain vertical when walking or falling.
	return IsMovingOnGround() || IsFalling();
}

void UMyPawnMovementComponent::PhysicsRotation(float DeltaTime) {
...
void UMyPawnMovementComponent::PhysicsVolumeChanged(APhysicsVolume* NewVolume) {
	if (!HasValidData()) {
		return;
	}
...
bool UMyPawnMovementComponent::ShouldJumpOutOfWater(FVector& JumpDir) {
...
void UMyPawnMovementComponent::JumpOutOfWater(FVector WallNormal) {}


bool UMyPawnMovementComponent::CheckWaterJump(FVector CheckPoint, FVector& WallNormal) {
	if (!HasValidData()) {
		return false;
	}
...
void UMyPawnMovementComponent::AddImpulse(FVector Impulse, bool bVelocityChange) {
...
void UMyPawnMovementComponent::AddForce(FVector Force) {
...
void UMyPawnMovementComponent::MoveSmooth(const FVector& InVelocity, const float DeltaSeconds, FStepDownResult* OutStepDownResult) {
	if (!HasValidData()) {
		return;
	}
...
bool UMyPawnMovementComponent::IsWalkable(const FHitResult& Hit) const {
	if (!Hit.IsValidBlockingHit()) {
...
void UMyPawnMovementComponent::SetWalkableFloorAngle(float InWalkableFloorAngle) {
...
void UMyPawnMovementComponent::SetWalkableFloorZ(float InWalkableFloorZ) {
	WalkableFloorZ = FMath::Clamp(InWalkableFloorZ, 0.f, 1.f);
	WalkableFloorAngle = FMath::RadiansToDegrees(FMath::Acos(WalkableFloorZ));
}

float UMyPawnMovementComponent::K2_GetWalkableFloorAngle() const {
	return GetWalkableFloorAngle();
}

float UMyPawnMovementComponent::K2_GetWalkableFloorZ() const {
	return GetWalkableFloorZ();
}



bool UMyPawnMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const {
...
void UMyPawnMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult) const {
...
void UMyPawnMovementComponent::FindFloor(const FVector& CapsuleLocation, FFindFloorResult& OutFloorResult, bool bZeroDelta, const FHitResult* DownwardSweepResult) const {
...
void UMyPawnMovementComponent::K2_FindFloor(FVector CapsuleLocation, FFindFloorResult& FloorResult) const {
	FindFloor(CapsuleLocation, FloorResult, false);
}

void UMyPawnMovementComponent::K2_ComputeFloorDist(FVector CapsuleLocation, float LineDistance, float SweepDistance, float SweepRadius, FFindFloorResult& FloorResult) const {
	if (HasValidData()) {
...
bool UMyPawnMovementComponent::FloorSweepTest(
	FHitResult& OutHit,
	const FVector& Start,
	const FVector& End,
...
bool UMyPawnMovementComponent::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit) const {
	if (!Hit.bBlockingHit) {
		return false;
	}
...
bool UMyPawnMovementComponent::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const {
...
float UMyPawnMovementComponent::GetPerchRadiusThreshold() const {
	// Don't allow negative values.
	return FMath::Max(0.f, PerchRadiusThreshold);
}


float UMyPawnMovementComponent::GetValidPerchRadius() const {
	if (CharacterOwner) {
...
bool UMyPawnMovementComponent::ShouldComputePerchResult(const FHitResult& InHit, bool bCheckRadius) const {
...
bool UMyPawnMovementComponent::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FFindFloorResult& OutPerchFloorResult) const {
...
bool UMyPawnMovementComponent::CanStepUp(const FHitResult& Hit) const {
...
bool UMyPawnMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult &InHit, FStepDownResult* OutStepDownResult) {
...
void UMyPawnMovementComponent::HandleImpact(const FHitResult& Impact, float TimeSlice, const FVector& MoveDelta) {
	...
	if (PathFollowingComp.IsValid()) {	// Also notify path following!
...
void UMyPawnMovementComponent::ApplyImpactPhysicsForces(const FHitResult& Impact, const FVector& ImpactAcceleration, const FVector& ImpactVelocity) {
...
FString UMyPawnMovementComponent::GetMovementName() const {
	if (CharacterOwner) {
		if (CharacterOwner->GetRootComponent() && CharacterOwner->GetRootComponent()->IsSimulatingPhysics()) {
...
void UMyPawnMovementComponent::DisplayDebug(UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos) {
...
void UMyPawnMovementComponent::VisualizeMovement() const {
...
void UMyPawnMovementComponent::ForceReplicationUpdate() {
...
void UMyPawnMovementComponent::ForceClientAdjustment() {
	ServerLastClientAdjustmentTime = -1.f;
}

FVector UMyPawnMovementComponent::ConstrainInputAcceleration(const FVector& InputAcceleration) const {
...
FVector UMyPawnMovementComponent::ScaleInputAcceleration(const FVector& InputAcceleration) const {
	return GetMaxAcceleration() * InputAcceleration.GetClampedToMaxSize(1.0f);
}


FVector UMyPawnMovementComponent::RoundAcceleration(FVector InAccel) const {
...
float UMyPawnMovementComponent::ComputeAnalogInputModifier() const {
	const float MaxAccel = GetMaxAcceleration();
...
float UMyPawnMovementComponent::GetAnalogInputModifier() const {
	return AnalogInputModifier;
}

static uint32 s_WarningCount = 0;
float UMyPawnMovementComponent::GetSimulationTimeStep(float RemainingTime, int32 Iterations) const {
...
void UMyPawnMovementComponent::SmoothCorrection(const FVector& OldLocation, const FQuat& OldRotation, const FVector& NewLocation, const FQuat& NewRotation) {
...
void UMyPawnMovementComponent::SmoothClientPosition(float DeltaSeconds) {
...
void UMyPawnMovementComponent::SmoothClientPosition_Interpolate(float DeltaSeconds) {
...
void UMyPawnMovementComponent::SmoothClientPosition_UpdateVisuals() {
...
bool UMyPawnMovementComponent::ClientUpdatePositionAfterServerUpdate() {
...
void UMyPawnMovementComponent::ForcePositionUpdate(float DeltaTime) {
...
FNetworkPredictionData_Client* UMyPawnMovementComponent::GetPredictionData_Client() const {
...
FNetworkPredictionData_Server* UMyPawnMovementComponent::GetPredictionData_Server() const {
...
FNetworkPredictionData_Client_Character* UMyPawnMovementComponent::GetPredictionData_Client_Character() const {
...
FNetworkPredictionData_Server_Character* UMyPawnMovementComponent::GetPredictionData_Server_Character() const {
...
bool UMyPawnMovementComponent::HasPredictionData_Client() const {
	return (ClientPredictionData != nullptr) && HasValidData();
}

bool UMyPawnMovementComponent::HasPredictionData_Server() const {
	return (ServerPredictionData != nullptr) && HasValidData();
}

void UMyPawnMovementComponent::ResetPredictionData_Client() {
...
void UMyPawnMovementComponent::ResetPredictionData_Server() {
	ForceClientAdjustment();
	if (ServerPredictionData) {
		delete ServerPredictionData;
...
void UMyPawnMovementComponent::ReplicateMoveToServer(float DeltaTime, const FVector& NewAcceleration) {
...
void UMyPawnMovementComponent::CallServerMove
(
...
void UMyPawnMovementComponent::ServerMoveOld_Implementation
(
	float OldTimeStamp,
...
void UMyPawnMovementComponent::ServerMoveDual_Implementation(
	float TimeStamp0,
...
void UMyPawnMovementComponent::ServerMoveDualHybridRootMotion_Implementation(
	float TimeStamp0,
	FVector_NetQuantize10 InAccel0,
	uint8 PendingFlags,
	uint32 View0,
...
bool UMyPawnMovementComponent::VerifyClientTimeStamp(float TimeStamp, FNetworkPredictionData_Server_Character& ServerData) {
...
void UMyPawnMovementComponent::ProcessClientTimeStampForTimeDiscrepancy(float ClientTimeStamp, FNetworkPredictionData_Server_Character& ServerData) {
	// Should only be called on server in network games
	check(CharacterOwner != NULL);
	check(CharacterOwner->Role == ROLE_Authority);
	checkSlow(GetNetMode() < NM_Client);
	...
	if (GameNetworkManager != nullptr && GameNetworkManager->bMovementTimeDiscrepancyDetection && bServerMoveHasOccurred) {
		...
		if (ServerData.bResolvingTimeDiscrepancy) {
			...
			DeltaTimeAfterPayback = FMath::Max(DeltaTimeAfterPayback, UMyPawnMovementComponent::MIN_TICK_TIME);
...
bool UMyPawnMovementComponent::IsClientTimeStampValid(float TimeStamp, const FNetworkPredictionData_Server_Character& ServerData, bool& bTimeStampResetDetected) const {
...
void UMyPawnMovementComponent::OnClientTimeStampResetDetected() {}

void UMyPawnMovementComponent::OnTimeDiscrepancyDetected(float CurrentTimeDiscrepancy, float LifetimeRawTimeDiscrepancy, float Lifetime, float CurrentMoveError) {
...
void UMyPawnMovementComponent::ServerMove(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 CompressedMoveFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode) {
...
void UMyPawnMovementComponent::ServerMove_Implementation(
	float TimeStamp,
	FVector_NetQuantize10 InAccel,
	FVector_NetQuantize100 ClientLoc,
	uint8 MoveFlags,
...
void UMyPawnMovementComponent::ServerMoveHandleClientError(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& RelativeClientLoc, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode) {
...
bool UMyPawnMovementComponent::ServerCheckClientError(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& ClientWorldLocation, const FVector& RelativeClientLocation, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode) {
...
bool UMyPawnMovementComponent::ServerMove_Validate(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 MoveFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode) {
	return true;
}

void UMyPawnMovementComponent::ServerMoveDual(float TimeStamp0, FVector_NetQuantize10 InAccel0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode) {
	if (ClientMovementBase != nullptr) {
...
bool UMyPawnMovementComponent::ServerMoveDual_Validate(float TimeStamp0, FVector_NetQuantize10 InAccel0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode) {
	return true;
}

void UMyPawnMovementComponent::ServerMoveDualHybridRootMotion(float TimeStamp0, FVector_NetQuantize10 InAccel0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode) {
...
bool UMyPawnMovementComponent::ServerMoveDualHybridRootMotion_Validate(float TimeStamp0, FVector_NetQuantize10 InAccel0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode) {
	return true;
}

void UMyPawnMovementComponent::ServerMoveOld(float OldTimeStamp, FVector_NetQuantize10 OldAccel, uint8 OldMoveFlags) {
	CharacterOwner->ServerMoveOld(OldTimeStamp, OldAccel, OldMoveFlags);
}

bool UMyPawnMovementComponent::ServerMoveOld_Validate(float OldTimeStamp, FVector_NetQuantize10 OldAccel, uint8 OldMoveFlags) {
	return true;
}



void UMyPawnMovementComponent::MoveAutonomous
(
...
void UMyPawnMovementComponent::UpdateFloorFromAdjustment() {
	if (!HasValidData()) {
		return;
	}
...
void UMyPawnMovementComponent::SendClientAdjustment() {
	if (!HasValidData()) {
		return;
	}
...
void UMyPawnMovementComponent::ClientVeryShortAdjustPosition(float TimeStamp, FVector NewLoc, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode) {
...
void UMyPawnMovementComponent::ClientVeryShortAdjustPosition_Implementation
(
	float TimeStamp,
	FVector NewLoc,
...
void UMyPawnMovementComponent::ClientAdjustPosition(float TimeStamp, FVector NewLoc, FVector NewVel, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode) {
...
void UMyPawnMovementComponent::ClientAdjustPosition_Implementation
(
	float TimeStamp,
	FVector NewLocation,
	FVector NewVelocity,
...
void UMyPawnMovementComponent::ClientAdjustRootMotionPosition(float TimeStamp, float ServerMontageTrackPosition, FVector ServerLoc, FVector_NetQuantizeNormal ServerRotation, float ServerVelZ, UPrimitiveComponent* ServerBase, FName ServerBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode) {
...
void UMyPawnMovementComponent::OnClientCorrectionReceived(FNetworkPredictionData_Client_Character& ClientData, float TimeStamp, FVector NewLocation, FVector NewVelocity, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode) {
#if !UE_BUILD_SHIPPING
	if (CharacterMovementCVars::NetShowCorrections != 0) {
...
void UMyPawnMovementComponent::ClientAdjustRootMotionPosition_Implementation(
	float TimeStamp,
...
void UMyPawnMovementComponent::ClientAdjustRootMotionSourcePosition(float TimeStamp, FRootMotionSourceGroup ServerRootMotion, bool bHasAnimRootMotion, float ServerMontageTrackPosition, FVector ServerLoc, FVector_NetQuantizeNormal ServerRotation, float ServerVelZ, UPrimitiveComponent* ServerBase, FName ServerBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode) {
...
void UMyPawnMovementComponent::ClientAdjustRootMotionSourcePosition_Implementation(
	float TimeStamp,
...
void UMyPawnMovementComponent::ClientAckGoodMove(float TimeStamp) {
	CharacterOwner->ClientAckGoodMove(TimeStamp);
}

void UMyPawnMovementComponent::ClientAckGoodMove_Implementation(float TimeStamp) {
	if (!HasValidData() || !IsActive()) {
		return;
	}
...
void UMyPawnMovementComponent::CapsuleTouched(UPrimitiveComponent* OverlappedComp, AActor* Other, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult) {
	if (!bEnablePhysicsInteraction) {
		return;
	}
...
void UMyPawnMovementComponent::SetAvoidanceGroup(int32 GroupFlags) {
	AvoidanceGroup.SetFlagsDirectly(GroupFlags);
}

void UMyPawnMovementComponent::SetAvoidanceGroupMask(const FNavAvoidanceMask& GroupMask) {
	AvoidanceGroup.SetFlagsDirectly(GroupMask.Packed);
}

void UMyPawnMovementComponent::SetGroupsToAvoid(int32 GroupFlags) {
	GroupsToAvoid.SetFlagsDirectly(GroupFlags);
}

void UMyPawnMovementComponent::SetGroupsToAvoidMask(const FNavAvoidanceMask& GroupMask) {
	GroupsToAvoid.SetFlagsDirectly(GroupMask.Packed);
}

void UMyPawnMovementComponent::SetGroupsToIgnore(int32 GroupFlags) {
	GroupsToIgnore.SetFlagsDirectly(GroupFlags);
}

void UMyPawnMovementComponent::SetGroupsToIgnoreMask(const FNavAvoidanceMask& GroupMask) {
	GroupsToIgnore.SetFlagsDirectly(GroupMask.Packed);
}

void UMyPawnMovementComponent::SetAvoidanceEnabled(bool bEnable) {
...
void UMyPawnMovementComponent::ApplyDownwardForce(float DeltaSeconds) {
...
void UMyPawnMovementComponent::ApplyRepulsionForce(float DeltaSeconds) {
...
void UMyPawnMovementComponent::ApplyAccumulatedForces(float DeltaSeconds) {
...
void UMyPawnMovementComponent::ClearAccumulatedForces() {
	PendingImpulseToApply = FVector::ZeroVector;
	PendingForceToApply = FVector::ZeroVector;
	PendingLaunchVelocity = FVector::ZeroVector;
}

void UMyPawnMovementComponent::AddRadialForce(const FVector& Origin, float Radius, float Strength, enum ERadialImpulseFalloff Falloff) {
...
void UMyPawnMovementComponent::AddRadialImpulse(const FVector& Origin, float Radius, float Strength, enum ERadialImpulseFalloff Falloff, bool bVelChange) {
	FVector Delta = UpdatedComponent->GetComponentLocation() - Origin;
	const float DeltaMagnitude = Delta.Size();
...
void UMyPawnMovementComponent::RegisterComponentTickFunctions(bool bRegister) {
...
void UMyPawnMovementComponent::ApplyWorldOffset(const FVector& InOffset, bool bWorldShift) {
...
void UMyPawnMovementComponent::TickCharacterPose(float DeltaTime) {
	...
	if (CharacterMesh->ShouldTickPose()) {
		...
		if (CharacterOwner->IsPlayingRootMotion() || bWasPlayingRootMotion) {
			...
			{
				FAnimMontageInstance* RootMotionMontageInstance = CharacterOwner->GetRootMotionAnimMontageInstance();
				UE_LOG(LogRootMotion, Log, TEXT("UMyPawnMovementComponent::TickCharacterPose Role: %s, RootMotionMontage: %s, MontagePos: %f, DeltaTime: %f, ExtractedRootMotion: %s, AccumulatedRootMotion: %s")
...
bool UMyPawnMovementComponent::HasRootMotionSources() const {
...
uint16 UMyPawnMovementComponent::ApplyRootMotionSource(FRootMotionSource* SourcePtr) {
	if (SourcePtr != nullptr) {
		...
		checkf(false, TEXT("Passing nullptr into UMyPawnMovementComponent::ApplyRootMotionSource"));
	}

	return (uint16)ERootMotionSourceID::Invalid;
}

void UMyPawnMovementComponent::OnRootMotionSourceBeingApplied(const FRootMotionSource* Source) {}

TSharedPtr<FRootMotionSource> UMyPawnMovementComponent::GetRootMotionSource(FName InstanceName) {
	return CurrentRootMotion.GetRootMotionSource(InstanceName);
}

TSharedPtr<FRootMotionSource> UMyPawnMovementComponent::GetRootMotionSourceByID(uint16 RootMotionSourceID) {
	return CurrentRootMotion.GetRootMotionSourceByID(RootMotionSourceID);
}

void UMyPawnMovementComponent::RemoveRootMotionSource(FName InstanceName) {
	CurrentRootMotion.RemoveRootMotionSource(InstanceName);
}

void UMyPawnMovementComponent::RemoveRootMotionSourceByID(uint16 RootMotionSourceID) {
	CurrentRootMotion.RemoveRootMotionSourceByID(RootMotionSourceID);
}

void UMyPawnMovementComponent::ConvertRootMotionServerIDsToLocalIDs(const FRootMotionSourceGroup& LocalRootMotionToMatchWith, FRootMotionSourceGroup& InOutServerRootMotion, float TimeStamp) {
...
bool UMyPawnMovementComponent::CanDelaySendingMove(const FSavedMovePtr& NewMove) {
	return true;
}

float UMyPawnMovementComponent::GetClientNetSendDeltaTime(const APlayerController* PC, const FNetworkPredictionData_Client_Character* ClientData, const FSavedMovePtr& NewMove) const {
...
void UMyPawnMovementComponent::UpdateFromCompressedFlags(uint8 Flags) {
	if (!CharacterOwner) {
		return;
	}
...
void UMyPawnMovementComponent::FlushServerMoves() {
